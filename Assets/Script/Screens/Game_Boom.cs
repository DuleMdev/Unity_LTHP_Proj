...
public class Game_Boom : TrueGameAncestor
{
    ...
    TEXDraw questionTEXDraw;// A kérdés kiírásához a Text komponens
    ...
    int tvScreenIndex = 0;  // A következő (jó) válasz melyik helyre kerül
    ...
    new protected void Awake()
    {
        ...
        questionText = Common.SearchGameObject(gameObject, "screenQuestionText").GetComponent<Text>();
        questionTEXDraw = Common.SearchGameObject(gameObject, "screenQuestionTEXDraw").GetComponent<TEXDraw>();
    ...
    override public IEnumerator PrepareTask()
    {
        ...
        {
            if (item.isImage) // answer[0] == '#')
            {
                //Sprite sprite = Common.menuLessonPlan.GetPicture(item.Substring(1));

                Sprite sprite;
                if (Common.configurationController.isServer2020)
                    sprite = taskData.gameData.GetSprite(item.answer);
                else
                    sprite = taskData.gameData.GetSprite(item.answer.Substring(1));
        ...
        if (Common.configurationController.isServer2020)
            if (Common.configurationController.answerFeedback != ConfigurationController.AnswerFeedback.Immediately)
                holdingNeed = taskData.answers.Count;
            else
                holdingNeed = (taskData.needGoodAnswers) ? taskData.goodAnswerPiece : taskData.answers.Count - taskData.goodAnswerPiece;
        else
            if (Common.configurationController.answerFeedback != ConfigurationController.AnswerFeedback.Immediately)
                holdingNeed = taskData.goodAnswers.Count + taskData.wrongAnswers.Count;
            else
                holdingNeed = (taskData.needGoodAnswers) ? taskData.goodAnswers.Count : taskData.wrongAnswers.Count;
    ...
    override public IEnumerator ScreenShowFinishCoroutine()
    {
        ...
        if (status == Status.Exit) yield break;
        ...
        // Megjelennek a válaszok helyei, egymás után felugranak
        if (status == Status.Exit) yield break;
        foreach (TVScreen tvScreen in tvScreenList)
        {
            ...
            Common.audioController.SFXPlay("boing");
            yield return new WaitForSeconds(taskData.animSpeed2); // Várunk 2 tizedmásodpercet
        }
        ...
        if (firstTime)
        {
            tvScreen.TVOn(taskData.animSpeed1 * 2);
    ...
    void EvaluateCoroutine(JSONNode jsonData)
    {
        if (Common.configurationController.answerFeedback != ConfigurationController.AnswerFeedback.Immediately)
        {
            ...
            {
                TaskBoomData.AnswerData answerData = taskData.GetAnswerDataFromID(jsonData[C.JSONKeys.selectedAnswer].Value);

                tvScreenList[tvScreenIndex].ShowAnswer(answerData.answer, answerData.isImage && pictures.ContainsKey(answerData.answer) ? pictures[answerData.answer] : null);
                tvScreenIndex++;
            }
        }
        else
        {
            ...
            TaskBoomData.AnswerData answerData = taskData.GetAnswerDataFromID(answer);
            ShowItemInTV(answerData.answer, answerData.isImage);
            ...
            switch (jsonData[C.JSONKeys.evaluateAnswer].Value)
            {
                case C.JSONValues.evaluateIsTrue:
                    ...
                    // Megjelenik a jó válasz a válaszok között
                    //tvScreenList[answerIndex].BackgroundGreen(); // Nem kell megváltoztatni a háttér színét, jó az fehéren is.
                    if (Common.configurationController.isServer2020)
                        tvScreenList[answerIndex].ShowAnswer(answerData.answer, answerData.isImage && pictures.ContainsKey(answerData.answer) ? pictures[answerData.answer] : null);
                    else
        ...
        if (networkEventType == NetworkEventType.DataEvent)
        {
            switch (jsonNodeMessage[C.JSONKeys.gameEventType])
            {
                ...
                case C.JSONValues.show:
                    ...
                    string showItem = jsonNodeMessage[C.JSONKeys.showItem].Value;
                    bool showItemIsPicture = jsonNodeMessage[C.JSONKeys.showItemIsPicture].AsBool;
                    ShowItemInTV(showItem, showItemIsPicture);
