...
public class Game_MathMonster : GameDragAncestor {

    [Tooltip("A buborékokat véletlenszerű színnel színezze ?")]
    public bool changeBubbleColor = true;

    [Tooltip("Kérdések közti távolság")]
    public float distanceBetweenQuestion;

    GameObject questionPrefab;
    GameObject bubbleGrabPrefab;
    ...
    override public void Awake()
    {
        ...
        questionPrefab = gameObject.SearchChild("TextMeshWithBubbleDragTarget").gameObject;
        bubbleGrabPrefab = gameObject.SearchChild("BubbleGrab").gameObject;
    ...
    override public IEnumerator PrepareTask()
    {
        ...
        texDrawScreenQuestion.text = taskData.taskDescription;
        ...
        {
            GameObject bubble = Instantiate(bubbleGrabPrefab); //, bubblesRoot.transform, true);
            //bubble.GetComponentInChildren<BubbleDrag>().Awake();
        ...
        for (int i = 0; i < taskData.questions.Count; i++)
        {
            ...
            textMeshWithDragTarget.Initialize(taskData.questions[i], i);
        ...
        */
        ...
        actQuestionRoot.transform.position = actQuestionRoot.transform.position.SetX(Camera.main.aspect * -3);
