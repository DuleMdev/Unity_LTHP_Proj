...
public class Game_Fish_With_Layout : GameDragAncestor { 
    ...
    List<FishDrag_With_Layout> listOfFishDrag;  // A válasz halakat tartalmazó tömb
    ...
    override public IEnumerator PrepareTask()
    {
        ...
        textMeshWithDragTarget.Initialize(taskData.questions[0], 0, layoutManager.GetSprite("answer"));
        ...
        textMeshWithDragTarget.transform.localPosition = Vector3.zero;
        textMeshWithDragTarget.transform.localScale = Vector3.one;
        textMeshWithDragTarget.Initialize((taskData.questions.Count > 1) ? taskData.questions[1] : new TextMeshWithDragTargetData(""), 1, layoutManager.GetSprite("answer")); // Ha nincs második kérdés, akkor egy üres stringgel inicializáljuk
        questions.Add(textMeshWithDragTarget);
        ...
        */
        ...
        listOfFishDrag = new List<FishDrag_With_Layout>();
        List<string> answers = taskData.GetAnswers();
        for (int i = 0; i < answers.Count; i++)
        {
            FishDrag_With_Layout fishDrag = Instantiate(fishPrefab).GetComponent<FishDrag_With_Layout>();
            //fishDrag.transform.parent = fishRoot.transform;
            fishDrag.transform.SetParent(fishRoot.transform, true);
            fishDrag.Initialize(i % 6 + ((LayoutManager.actLayoutSetName == "1") ? 0 : 6), answers[i]);
        ...
        fishAreaMove.transform.position = new Vector3(Camera.main.aspect * -2 * fishAreaMove.transform.lossyScale.x, fishAreaMove.transform.position.y, fishAreaMove.transform.position.z);
    ...
    */
    ...
    override public void MessageArrived(NetworkEventType networkEventType, int connectionId, JSONNode jsonNodeMessage)
    {
        ...
        if (networkEventType == NetworkEventType.DataEvent)
        {
            if (jsonNodeMessage[C.JSONKeys.dataContent].Value == C.JSONValues.gameEvent)
            {
                ...
                foreach (FishDrag_With_Layout fishDrag in listOfFishDrag)
                {
    ...
    void SetPictures()
    {
        ...
        // Kicseréljük a halak képeit a beállított design-nak megfelelően
        for (int i = 0; i < listOfFishDrag.Count; i++)
        {
            listOfFishDrag[i].ChangeFish(i % 6 + ((LayoutManager.actLayoutSetName == "1") ? 0 : 6));
        }
