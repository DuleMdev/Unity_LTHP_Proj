...
public class Game_Hangman_With_Layout : TrueGameAncestor
{
    ...
    override public IEnumerator PrepareTask()
    {
        ...
        abcTable.Init(KeyButtonClick, taskData.keyset); // Common.languageController.actLanguageID); // A készülék nyelve határozza meg a billentyűzetet
        ...
        foreach (string lineText in lines)
        {
            ...
            foreach (char letter in lineText)
            {
                if (letter != ' ')
                {
                    Letter newLetter = Instantiate(LetterPrefab.gameObject).GetComponent<Letter>();
    ...
    float GetLetterSize(string text) {
        ...
        float size = availableArea.y / letterPrefab.GetHeight();
        float goodSize = 0;
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            int lineLength = (int)((availableArea.x - letterPrefab.GetWidth() * (goodSize + size)) / 
                ((letterPrefab.GetWidth() + letterPrefab.GetWidth() * distanceBetweenChars) * (goodSize + size))) + 1;
            int rowCount = (int)((availableArea.y - letterPrefab.GetHeight() * (goodSize + size)) / 
                ((letterPrefab.GetHeight() + letterPrefab.GetWidth() * distanceBetweenChars) * (goodSize + size))) + 1;
            ...
            { // Ha sikerült, akkor a méret megfelelő
                goodSize += size;
                this.lines = lines;
                Debug.Log("new size : " + goodSize + " | line : " + lineLength + " | row : " + rowCount);
            }
            else {
                Debug.Log("wrong size : " + (goodSize + size) + " | line : " + lineLength + " | row : " + rowCount);
            }

            size /= 2; // Tovább próbálkozunk egy kisebb méret hozzáadásával
        }
    ...
    */
    ...
    void SetPictures()
    {
        ...
        monster.SetPictures(layoutManager);

        abcTable.SetPictures(layoutManager.GetSprite("abcTable"));

        foreach (Letter letter in listOfLetters)
        {
            letter.SetPictures(layoutManager.GetSprite("answer"), layoutManager.GetColor("answer"));
        }
