...
public class Game_Bubble : TrueGameAncestor
{
    GameObject bubblePrefab;

    GameObject questionMove;            // A kérdés előugrásához kell
    Text textQuestion;                  // A kérdés szövegének kiírásához
    TEXDraw texDrawQuestion;
    Text textNotice;                    // Segítség a játékhoz (pukkaszd ki a rossz választ tartalmazó buborékokat)
    TEXDraw texDrawNotice;

    RectTransform canvasQuestionPicture;// Ezt a gameObject-et kell nagyítani és a rectTransformját méretezni, hogy méret arányos maradjon a kép
    ...
    override public void Awake()
    {
        ...
        bubblePrefab = gameObject.SearchChild("BubblePrefab").gameObject;
        ...
        texDrawQuestion = Common.SearchGameObject(gameObject, "TEXDrawQuestion").GetComponent<TEXDraw>();
        textNotice = Common.SearchGameObject(gameObject, "textNotice").GetComponent<Text>();
        texDrawNotice = Common.SearchGameObject(gameObject, "TEXDrawNotice").GetComponent<TEXDraw>();
        particleSystemPopOut = Common.SearchGameObject(gameObject, "bubbleBursts").GetComponent<ParticleSystem>();

        canvasQuestionPicture = gameObject.SearchChild("CanvasQuestionPicture").GetComponent<RectTransform>();
    ...
    override public IEnumerator PrepareTask()
    {
        ...
        if (!string.IsNullOrEmpty(taskData.questionPicture))
        {
            ...
            Rect spriteSize = imageQuestionPicture.sprite.rect;
        ...
        // A kérdés és a megjegyzés szövegét eljelyezzük a megfelelő komponensekben
        // Választunk a Text és a TEXDraw komponens között, hogy melyik látszódjon
        textQuestion.text = taskData.question;
        texDrawQuestion.text = taskData.question;
        textQuestion.gameObject.SetActive(Common.configurationController.textComponentType == ConfigurationController.TextComponentType.Text);
        texDrawQuestion.gameObject.SetActive(Common.configurationController.textComponentType == ConfigurationController.TextComponentType.TEXDraw);
        ...
        string noticeText = "(" + ((string.IsNullOrEmpty(taskData.taskText)) ? Common.languageController.Translate("PopOutWrongAnswers") : taskData.taskText) + ")";
        textNotice.text = noticeText;
        texDrawNotice.text = noticeText;
        textNotice.gameObject.SetActive(Common.configurationController.textComponentType == ConfigurationController.TextComponentType.Text);
        texDrawNotice.gameObject.SetActive(Common.configurationController.textComponentType == ConfigurationController.TextComponentType.TEXDraw);
        ...
        foreach (int answerID in taskData.GetAnswers())
        {
            ...
            GameObject newBubbleGameObject = GameObject.Instantiate(bubblePrefab, background.transform, true);
            newBubbleGameObject.SetActive(true); // Egy pillanatra bekapcsoljuk, hogy le tudjon futni az Awake metódusa
            ...
            Color newColor = new Color(
                (float)(Common.random.NextDouble() * 0.5 + 0.3),
                (float)(Common.random.NextDouble() * 0.5 + 0.3),
                (float)(Common.random.NextDouble() * 0.5 + 0.3),
                1f
                );
    ...
    */
        ...
        foreach (Bubble2 bubble in listOfBubble)
        {
            if (!bubble.gameObject.activeSelf) // Ha nem látható megjelenítjük
            {
                ...
                Vector2 randomPosition = new Vector2(
                    bubbleBirthPos.transform.position.x + Random.Range(-0.1f, 0.1f),
                    bubbleBirthPos.transform.position.y + Random.Range(-0.1f, 0.1f)
                    );
                yield return StartCoroutine(bubble.InitializeServer2020(position: randomPosition));   // Beállítjuk a buborék kezdeti pozícióját
    ...
    {
        if (status == Status.Play && !zoomer.active)
        ...
        if (networkEventType == NetworkEventType.DataEvent)
        {
            switch (jsonNodeMessage[C.JSONKeys.gameEventType])
            {
                case C.JSONValues.answer:
                    ...
                    // Kipukasztjuk az elküldött felíratú buborékot az aktívak közül, de csak akkor ha a válasz jó vagy rossz
                    if (jsonNodeMessage[C.JSONKeys.evaluateAnswer].Value == C.JSONValues.evaluateIsFalse ||
                        jsonNodeMessage[C.JSONKeys.evaluateAnswer].Value == C.JSONValues.evaluateIsTrue)
                    {
                        foreach (Bubble2 bubble in listOfBubble)
    ...
    */
    ...
    IEnumerator VerifyCoroutine()
    {
        ...
        if (liveBubble == taskData.goodAnswerPiece)
        {
            ...
            foreach (Bubble2 bubble in listOfBubble)
                if (bubble.gameObject.activeSelf)
                    if (!taskData.goodAnswers.Contains(bubble.text.text))  // Itt az indexet kellene ellenőrízni nem a text-tet
                    {
                        goodAnswer = false;
