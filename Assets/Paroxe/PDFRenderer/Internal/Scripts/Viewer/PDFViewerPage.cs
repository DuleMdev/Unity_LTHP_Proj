...
#if UNITY_WEBGL
    public class PDFViewerPage : UIBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
    {
        [SerializeField]
        private Texture2D m_HandCursor;
        ...
        Game_PDF _gamePDF;
        public Game_PDF gamePDF
        {
            get {
                if (_gamePDF == null)
                    _gamePDF = transform.GetComponentInParent<Game_PDF>();
        ...
        private Camera canvasCamera
        {
            get {
                if (m_CanvasCamera == null)
                    m_CanvasCamera = transform.GetComponentInParent<Canvas>().worldCamera;
        ...
        public void OnPointerClick(PointerEventData eventData)
        {
            ...
            string link = GetLinkAtPoint(eventData.pressPosition, eventData.pressEventCamera);

            if (link != null)
#if UNITY_EDITOR
                Application.OpenURL(link); // Link megnyitása böngészőben
#else
                Application.ExternalEval("window.open(\"" + link + "\")"); // Link megnyitása böngészőben egy új ablakban
#endif
        }
        ...
        private void Update()
        {
            if (m_PointerInside)
            {
                if (m_HandCursor == null || !pdfViewer.ChangeCursorWhenOverURL)
                    return;

                string link = GetLinkAtPoint(Input.mousePosition, canvasCamera);

                SetCursorPointer(link != null);
            }
        }

        void SetCursorPointer(bool handCursor)
        {
            if (handCursor)
        ...
        private string GetLinkAtPoint(Vector2 point, Camera camera)
        {
                ...
                out localPointerPosition))
            {
                ...
                localPointerPosition = new Vector2(
                    (localPointerPosition.x + rt.sizeDelta.x * 0.5f) / rt.sizeDelta.x,
                    (localPointerPosition.y + rt.sizeDelta.y * 0.5f) / rt.sizeDelta.y);

                return gamePDF.taskData.getUrl(transform.GetSiblingIndex(), localPointerPosition);
            }
